...
class BitcoinProvider : BaseProvider(DeFiWalletSDK.currWallet()) {
    ...
    override fun getTransactions(
        ...
        return flow {
            val result = blockChairService.bitcoinRelatedInfoByAddress(address, limit, offset)
                .data.values.first().transactionsHash
            emit(result.joinToString(separator = ","))
        }.flatMapConcat { it ->
            flow {
                val result = blockChairService.bitcoinTxsByHash(it)
                    .data.values.map { it ->
    ...
    override fun buildTransactionPlan(sendModel: SendModel): Flow<SendPlanModel> {
        ...
        return flow {
            val info = blockChairService.bitcoinRelatedInfoByAddress(from).data.values.first()
            emit(info)
        }.map {
